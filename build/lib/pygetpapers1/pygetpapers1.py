...
class ApiPlugger:
    ...
    def _assist_warning_api(self):
        ...
        if (
            ...
            raise pygetpapers1Error("Please specify a query")
    ...
    def _add_date_to_query(self):
        """Builds query from simple dates in --startdate and --enddate. (See https://pygetpapers1.readthedocs.io/en/latest/index.html#download-papers-within-certain-start-and-end-date-range)
    ...
    def add_terms_from_file(self):
        ...
        if self.query_namespace[QUERY]:
            ...
            if self.query_namespace[TERMS]:
                ...
                raise pygetpapers1Error("Please provide a query with not")

    def check_query_logic_and_run(self):
        """Checks the logic in query_namespace and runs pygetpapers1 for the given query"""
        try:
            self._assist_warning_api()
        except pygetpapers1Error as err:
...
class pygetpapers1:
    ...
    def run_command(
        ...
        """Runs pygetpapers1 for the given parameters"""
        got_parameters = locals()
        if output == False:
            got_parameters[OUTPUT] = self.default_path
        self.runs_pygetpapers1_for_given_args(got_parameters)

    def runs_pygetpapers1_for_given_args(self, query_namespace):
        """Runs pygetpapers1 for flags described in a dictionary
        ...
        if query_namespace[VERSION]:
            logging.info("You are running pygetpapers1 version %s", self.version)
            return
        ...
        if query_namespace[API] not in list(self.download_tools.config):
            raise pygetpapers1Error("API not supported yet")
            return
    ...
    def create_argparser(self):
        ...
        parser = configargparse.ArgParser(
            description=f"Welcome to pygetpapers1 version {version}. -h or --help for help",
            add_config_file_help=False,
        )
        parser.add_argument(
            "--config",
            is_config_file=True,
            help="config file path to read query for pygetpapers1",
        )
        ...
        self.runs_pygetpapers1_for_given_args(self.query_namespace)


def main():
    """Runs the CLI"""
    callpygetpapers1 = pygetpapers1()
    callpygetpapers1.create_argparser()
